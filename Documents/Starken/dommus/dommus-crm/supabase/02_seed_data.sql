-- =====================================================
-- DOMMUS SMART HOME CRM - DADOS DE EXEMPLO
-- Execute este script APÓS o 01_create_tables.sql
-- =====================================================

-- =====================================================
-- 1. AGENTES (Vendedores)
-- =====================================================
INSERT INTO agents (id, name, email, phone, position, status)
VALUES
  (gen_random_uuid(), 'Carlos Eduardo Müller', 'carlos@dommus.com.br', '(47) 99999-0001', 'Consultor Sênior', 'active'),
  (gen_random_uuid(), 'Ana Paula Ritter', 'ana@dommus.com.br', '(47) 99999-0002', 'Consultora', 'active'),
  (gen_random_uuid(), 'Felipe Augusto Schmidt', 'felipe@dommus.com.br', '(47) 99999-0003', 'Consultor Técnico', 'active')
...
-- =====================================================
-- 3. LEADS (com status_id vinculado à tabela lead_statuses)
-- =====================================================
DO $$
DECLARE
  v_status_novo UUID;
  v_status_qualificado UUID;
  v_status_em_contato UUID;
  v_status_proposta UUID;
BEGIN
  -- Buscar IDs dos status
  SELECT id INTO v_status_novo FROM lead_statuses WHERE name = 'Novo' LIMIT 1;
  SELECT id INTO v_status_qualificado FROM lead_statuses WHERE name = 'Qualificado' LIMIT 1;
  SELECT id INTO v_status_em_contato FROM lead_statuses WHERE name = 'Em Contato' LIMIT 1;
  SELECT id INTO v_status_proposta FROM lead_statuses WHERE name = 'Proposta' LIMIT 1;

  -- Inserir leads com status_id
  INSERT INTO leads (id, name, company, email, phone, source, status, status_id, notes, position, address, website)
  VALUES
    (gen_random_uuid(), 'Lucas Ferreira', 'Ferreira Engenharia', 'lucas@ferreiraeng.com.br', '(47) 99876-5432', 'Instagram', 'Novo', v_status_novo, 'Viu nosso projeto do home theater e quer orçamento similar.', 'Engenheiro Civil', 'Rua das Acácias, 123, Itoupava Norte', 'www.ferreiraeng.com.br'),
    (gen_random_uuid(), 'Amanda Cristina Souza', NULL, 'amanda.souza@gmail.com', '(47) 99765-4321', 'Site', 'Qualificado', v_status_qualificado, 'Reformando apartamento 3 quartos. Interesse em iluminação inteligente e persianas.', 'Designer de Interiores', 'Av. Beira Rio, 890, Centro', NULL),
    (gen_random_uuid(), 'Eduardo Martins', 'EM Advocacia', 'eduardo@emadvocacia.com.br', '(47) 99654-3210', 'Indicação', 'Em Contato', v_status_em_contato, 'Indicado pelo Dr. Paulo. Quer automação no escritório de advocacia.', 'Advogado', 'Rua Nereu Ramos, 300, Centro', 'www.emadvocacia.com.br'),
    (gen_random_uuid(), 'Juliana Prado', 'Academia FitLife', 'juliana@fitlife.com.br', '(47) 99543-2109', 'Google', 'Proposta', v_status_proposta, 'Quer sistema de som e TVs para a academia. 500m².', 'Proprietária', 'Rua São Paulo, 1200, Victor Konder', 'www.fitlifeblumenau.com.br'),
    (gen_random_uuid(), 'Marcos Vinícius Lima', NULL, 'marcoslima@hotmail.com', '(47) 99432-1098', 'Facebook', 'Novo', v_status_novo, 'Interesse em câmeras de segurança e fechaduras inteligentes.', NULL, 'Rua Bahia, 678, Garcia', NULL),
    (gen_random_uuid(), 'Patrícia Oliveira', 'Oliveira Imóveis', 'patricia@oliveiraimoveis.com.br', '(47) 99321-0987', 'Feira', 'Qualificado', v_status_qualificado, 'Corretora interessada em parceria para decorados de empreendimentos.', 'Corretora', 'Av. Brasil, 2000, Centro', 'www.oliveiraimoveis.com.br'),
    (gen_random_uuid(), 'Rafael Santos', 'Tech Solutions SC', 'rafael@techsolutions.sc', '(47) 99210-9876', 'LinkedIn', 'Em Contato', v_status_em_contato, 'CEO de empresa de TI. Quer automação residencial completa.', 'CEO', 'Rua das Missões, 456, Itoupava Seca', 'www.techsolutions.sc'),
    (gen_random_uuid(), 'Isabela Fernandes', 'Salão Bella Donna', 'isabela@belladonna.com.br', '(47) 99987-6543', 'Site', 'Em Contato', v_status_em_contato, 'Quer iluminação especial para o salão de beleza.', 'Proprietária', 'Rua Sete de Setembro, 450, Centro', 'www.belladonnasc.com.br')
  ON CONFLICT DO NOTHING;
END $$;
...
VALUES
  ...
  (gen_random_uuid(), 'https://www.instagram.com/p/example7/', 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=800', NULL, 'Projeto corporativo em Joinville. Sala de reuniões com videoconferência e automação completa. #corporativo #salareuniao #automacao', 'image', 'Corporativo', true, 6, ARRAY['feed', 'projects'])
ON CONFLICT DO NOTHING;

-- =====================================================
-- 6. PROJETOS (vinculados aos clientes)
-- =====================================================
DO $$
DECLARE
...
BEGIN
  ...
  -- Inserir projetos
  IF v_client_roberto IS NOT NULL AND v_agent_carlos IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, start_date, estimated_end_date, address, city, state, agent_id, notes)
    VALUES ('Casa Mendes - Automação Completa', 'Projeto de automação residencial completa: iluminação, áudio multizona, climatização, segurança e home theater.', v_client_roberto, 'Em Execução', 'Residencial', 185000.00, '2024-01-15', '2024-04-30', 'Rua das Palmeiras, 245', 'Blumenau', 'SC', v_agent_carlos, 'Cliente premium, preferência por equipamentos Apple HomeKit.')
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_paulo IS NOT NULL AND v_agent_ana IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, start_date, estimated_end_date, address, city, state, agent_id, notes)
    VALUES ('Clínica Andrade - Automação', 'Automação do consultório médico: iluminação, climatização e áudio ambiente.', v_client_paulo, 'Aprovado', 'Comercial', 45000.00, '2024-02-01', '2024-03-15', 'Rua XV de Novembro, 890', 'Blumenau', 'SC', v_agent_ana, 'Projeto em parceria com arquiteta.')
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_keller IS NOT NULL AND v_agent_carlos IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, address, city, state, agent_id, notes)
    VALUES ('Residência Keller - Casa Nova', 'Casa em construção. Projeto completo de automação residencial.', v_client_keller, 'Orçamento', 'Residencial', 150000.00, 'Rua dos Ipês, 78', 'Blumenau', 'SC', v_agent_carlos, 'Aguardando aprovação do orçamento.')
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_carla IS NOT NULL AND v_agent_felipe IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, start_date, estimated_end_date, address, city, state, agent_id, notes)
    VALUES ('Home Theater Fontana', 'Sala de home theater dedicada 7.2.4. Projetor 4K, tela 120".', v_client_carla, 'Instalação', 'Residencial', 82000.00, '2024-01-08', '2024-02-20', 'Av. República Argentina, 567', 'Blumenau', 'SC', v_agent_felipe, 'Instalação em andamento.')
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_villa IS NOT NULL AND v_agent_ana IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, start_date, estimated_end_date, address, city, state, agent_id, notes)
    VALUES ('Villa Toscana - Som Ambiente', 'Sistema de som ambiente para restaurante: 8 zonas.', v_client_villa, 'Em Execução', 'Comercial', 38000.00, '2024-01-20', '2024-02-28', 'Rua Hermann Hering, 200', 'Blumenau', 'SC', v_agent_ana, 'Instalação aos finais de semana.')
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_thiago IS NOT NULL AND v_agent_carlos IS NOT NULL THEN
    INSERT INTO projects (name, description, client_id, status, type, estimated_value, start_date, estimated_end_date, address, city, state, agent_id, notes)
    VALUES ('Cobertura Ponta Aguda - Thiago', 'Cobertura duplex com automação completa.', v_client_thiago, 'Finalizado', 'Residencial', 220000.00, '2023-08-01', '2023-12-15', 'Rua Itajaí, 456, Cobertura', 'Blumenau', 'SC', v_agent_carlos, 'Projeto concluído. Cliente muito satisfeito.')
    ON CONFLICT DO NOTHING;
  END IF;
END $$;
...
BEGIN
  ...
  IF v_client_keller IS NOT NULL AND v_agent_carlos IS NOT NULL THEN
    INSERT INTO quotes (number, project_id, client_id, status, valid_until, subtotal, discount, discount_type, labor_cost, installation_cost, total_value, notes, terms, payment_conditions, agent_id)
    VALUES ('ORC-2024-001', v_project_keller, v_client_keller, 'Enviado', '2024-02-28', 128500.00, 5.0, 'percentage', 15000.00, 8000.00, 145075.00, 'Orçamento completo para automação residencial.', 'Garantia de 2 anos em todos os equipamentos.', '50% entrada, 30% na entrega, 20% na conclusão.', v_agent_carlos)
    ON CONFLICT DO NOTHING;
  END IF;

  IF v_client_carla IS NOT NULL AND v_agent_felipe IS NOT NULL THEN
    INSERT INTO quotes (number, project_id, client_id, status, valid_until, subtotal, discount, discount_type, labor_cost, installation_cost, total_value, notes, terms, payment_conditions, agent_id, approved_at)
    VALUES ('ORC-2024-002', v_project_fontana, v_client_carla, 'Aprovado', '2024-02-15', 68500.00, 3000.00, 'fixed', 8000.00, 5500.00, 79000.00, 'Home theater sala dedicada 7.2.4 canais.', 'Garantia de 2 anos. Inclui calibração.', '40% entrada, 40% na entrega, 20% após calibração.', v_agent_felipe, NOW())
    ON CONFLICT DO NOTHING;
  END IF;
END $$;
